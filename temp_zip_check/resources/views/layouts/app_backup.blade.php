    ...
    <div class="app-container">
                ...
                <ul class="sidebar-menu">
                    @if(auth()->user()->role === 'superadmin')
                        ...
                        <li class="sidebar-menu-item {{ request()->routeIs('superadmin.workspaces') ? 'active' : '' }}">
                            <a href="{{ route('superadmin.workspaces') }}"><i class='bx bx-buildings'></i><span>Create Workspaces</span></a>
                        </li>
        ...
        <main class="app-main" style="{{ !Auth::check() ? 'margin-left: 0; width: 100%;' : '' }}">
            @if(session()->has('impersonated_by'))
                <div style="background: var(--primary); color: white; padding: 12px; text-align: center; font-weight: 600; display: flex; justify-content: center; align-items: center; gap: 10px; z-index: 1000; position: sticky; top: 0; box-shadow: 0 4px 10px rgba(0,0,0,0.1); margin-bottom: 20px; border-radius: 0 0 16px 16px;">
                    ...
                    const dropdownHtml = `
                        <div id="global-notifications-dropdown" style="display: none; position: absolute; background: #ffffff; border: 1px solid #e2e8f0; border-radius: 8px; width: 320px; box-shadow: 0 10px 25px rgba(0,0,0,0.15); z-index: 99999; overflow: hidden;">
                            <div style="padding: 15px; border-bottom: 1px solid #e2e8f0; display: flex; justify-content: space-between; align-items: center;">
                                ...
                                ${data.unreadCount > 0 ? `<button id="mark-read-btn" style="background: none; border: none; color: #3b82f6; font-size: 12px; cursor: pointer; padding: 0;">Mark all as read</button>` : ''}
                            </div>
                            <div style="max-height: 350px; overflow-y: auto;">
                                ${data.notifications.length === 0 ? `<div style="padding: 20px; text-align: center; color: #64748b; font-size: 13px;">No notifications yet.</div>` : 
                                  data.notifications.map(notif => `
                                    <a href="${notif.data.url || '#'}" style="display: block; text-decoration: none; padding: 12px 15px; border-bottom: 1px solid #f1f5f9; background: ${notif.read_at ? '#ffffff' : '#f8fafc'}; transition: background 0.2s;" onmouseover="this.style.background='#f1f5f9'" onmouseout="this.style.background='${notif.read_at ? '#ffffff' : '#f8fafc'}'">
                                        <div style="font-size: 13px; font-weight: 600; color: #0f172a;">${notif.data.title || 'Notification'}</div>
                                        <div style="font-size: 12px; color: #475569; margin-top: 4px;">${notif.data.message || ''}</div>
