...
class AdminController extends Controller
{
    ...
    public function syncErpEmployees()
    {
        \Illuminate\Support\Facades\Artisan::call('sync:erp-employees');
        return redirect()->back()->with('success', 'Employees synced from ERP successfully!');
    }
    ...
    public function approveLeave(Leave $leave)
    {
        ...
        if ($leave->user) {
            $leave->user->notify(new \App\Notifications\GeneralNotification([
                'title'   => 'Leave Approved',
                'message' => 'Your leave (' . $leave->start_date . ' to ' . ($leave->end_date ?? $leave->start_date) . ') has been approved!',
                'type'    => 'leave_status',
                'url'     => '/employee/leave/apply',
            ]));
            try {
                $erpUrl = env('ERP_URL', 'http://127.0.0.1:5000');
                $erpSecret = env('ERP_SHARED_SECRET', 'default-erp-secret-12345');
                Http::withHeaders(['X-ERP-SECRET' => $erpSecret])->timeout(5)
                    ->post("{$erpUrl}/api/internal/sync-leave-status", [
                        'employee_email' => $leave->user->email,
                        'leave_id'       => $leave->id,
                        'status'         => 'approved',
                    ]);
            } catch (\Exception $e) {
                \Illuminate\Support\Facades\Log::warning('Leave ERP sync failed: ' . $e->getMessage());
            }
        }
    ...
    public function rejectLeave(Leave $leave)
    {
        ...
        if ($leave->user) {
            $leave->user->notify(new \App\Notifications\GeneralNotification([
                ...
                'message' => 'Your leave (' . $leave->start_date . ' to ' . ($leave->end_date ?? $leave->start_date) . ') has been rejected.',
                'type'    => 'leave_status',
                'url'     => '/employee/leave/apply',
            ]));
            try {
