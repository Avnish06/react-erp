...
<div class="content-header" style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 25px;">
    <div>
        <h1 style="font-size: 24px; font-weight: 700; color: var(--text-main); margin: 0;">All Queries</h1>
        <p style="color: var(--text-muted); margin: 5px 0 0 0;">View and manage queries across all workspaces</p>
    </div>
</div>
...
@if(session('error'))
    <div style="background: rgba(255, 71, 87, 0.1); border: 1px solid var(--danger); color: var(--danger); padding: 15px; border-radius: 12px; margin-bottom: 20px;">
        {{ session('error') }}
    </div>
@endif
...
<div class="card" style="margin-bottom: 25px;">
    <div class="card-header">
        <h2 style="font-size: 16px; font-weight: 600; margin: 0;">Employee Queries</h2>
    </div>
                    ...
                    @forelse($queries as $query)
                        <tr style="border-bottom: 1px solid var(--border-color);">
                            <td style="padding: 15px;">
                                ...
                                <div style="font-size: 11px; color: var(--primary); margin-top: 5px;">{{ $query->company->name }}</div>
                            </td>
                            <td style="padding: 15px;">
                            ...
                            <td style="padding: 15px; text-align: right;">
                                <button class="btn btn-sm btn-outline" onclick="openAssignModal({{ $query->id }}, '{{ $query->company_id }}')" style="margin-bottom: 5px;">Assign</button>
                                
                                <form action="{{ route('superadmin.queries.status', $query->id) }}" method="POST" style="display: inline-block;">
                                    @csrf
                                    @method('PATCH')
    ...
    <div class="card" style="width: 100%; max-width: 400px; margin: 20px; background: #ffffff; border-radius: 12px; overflow: hidden; box-shadow: 0 10px 30px rgba(0,0,0,0.1); animation: slideUp 0.3s ease-out;">
        ...
        <form id="assign-form" method="POST" style="padding: 25px;">
            ...
            <div class="form-group" style="margin-bottom: 25px;">
                ...
                <select name="assigned_to" id="assign-select" required class="search-input" style="width: 100%; padding: 10px 15px; border-radius: 8px; border: 1px solid #ddd; background: #fff; color: #333;">
                    <option value="">Select an employee...</option>
                    @foreach($allEmployees as $emp)
                        <option value="{{ $emp->id }}" data-company="{{ $emp->company_id }}">{{ $emp->name }} ({{ $emp->company->name }})</option>
                    @endforeach
                </select>
                <small style="color: #888; display: block; margin-top: 5px;">Only employees from the query's company should be assigned.</small>
            </div>
...
<script>
    function openAssignModal(queryId, companyId) {
        document.getElementById('assign-form').action = `/superadmin/queries/${queryId}/assign`;
        
        // Filter options by company_id visually (superadmin can see all, but should only assign to same company)
